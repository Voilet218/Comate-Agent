...
public class Agent {
    ...
    public String run(String userInput) {
        ...
        while (true) {
            // 3.1检查取消标记，这个标记会在用户按ctrl + c的时候被标记
            ...
            // 3.4检查预算，【token，迭代轮数，相同工具调用】，超出了就阻止
            ...
            try {
                ...
                // 调用 LLM，system+对话历史+工具定义+回调接口，每个sse返回块都是给到streamRenderer
                LlmClient.ChatResponse response = llmClient.chat(
                        conversationHistory,
                        toolDefinitions,
                        streamRenderer
                );
                // 3.7模型的思考添加到log，但是不渲染
                ...
                // 3.8buget记一下发送，响应，缓存命中，就是一个累加器
                budget.recordTokens(response.inputTokens(), response.outputTokens(), response.cachedInputTokens());

                // 3.9检查工具调用
                if (response.hasToolCalls()) {
                    // 这里是追加到一个sb上，等会儿给用户看的
                    ...
                    conversationHistory.add(LlmClient.Message.assistant(
                        // 这个是ds的特性，要求带上推理过程，要是其他模型会自己判断不追加reason
                            response.reasoningContent(),
                            response.content(),
                            response.toolCalls()
                    ));

                    // 清空render的缓存（在工具调用之前全部打印了）
                    ...
                    // 这里是工具返回的图片，处理之后也加到上下文，就不加到短期记忆了，图片太大了
    ...
    /**
     * 将检索到的长期记忆注入到 system prompt 中（替换 conversationHistory[0]）
     */
